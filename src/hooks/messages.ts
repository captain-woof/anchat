...
/*
export const useMessages = (roomId: string, initialMsgsToLoadNum: number = 100, loadNumMsgsWhilePaginating = 25) => {
    const [messagesNumToLoad, setMessagesNumToLoad] = useState<number>(initialMsgsToLoadNum)
    const [messages, pending, error] = useCollection(query(messagesRef, where('roomId', '==', roomId), limit(messagesNumToLoad), orderBy('timestamp', 'desc')))
    ...
    return {
        messages: structureMessages(messages),
        pending,
        error,
        loadMoreMsgs
    }
